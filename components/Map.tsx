...
const Map = () => {
  ...
  useEffect(() => {
    if (drivers.length > 0 && userLatitude != null && userLongitude != null) {
  ...
  if (!region) {
    return <View className="w-full h-[300px] rounded-lg bg-gray-200" />;
  }
  ...
  return (
    <View className="w-full h-[300px] rounded-lg overflow-hidden">
      <MapView
        ...
        initialRegion={region} // Pass the calculated region
      >
        {markers.map((marker) => (
          <Marker
            key={marker.id}
